...
- **Read odds through `BaseballResult.publishedOdds()` — it is now the only pre-game price a row has.** It returns the `ThreeWayOdds` actually posted before the game, or `null`, which means skip the game; there is deliberately no fallback. The row's other odds, `totalDiv`, is the payout of the side that *won*, so it is known only afterwards and is never a selection signal — `PickSettlement` pays at it, and because published rows were cross-checked against it, selection and settlement quote the same market. De-vig through `ThreeWayOdds.impliedHome/Draw/Away()` rather than rolling your own, so every family measures the house margin the same way. Historical note: reconstructed `HOME_DIV`/`DRAW_DIV`/`AWAY_DIV` columns used to sit alongside these. Which of the three was exact depended on the outcome, so any rule reading them partly learned the result (`docs/statistical-model-design.md` 설계 결정 D2, and a fake +35~55% ROI). They were dropped rather than merely documented — if you find a reference to them, it is stale.
