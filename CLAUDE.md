...
- **Stake sizing never lives inside `selectSlips`.** Selection is stateless and day-isolated by design; a path-dependent staking rule (e.g. martingale) implements `StakingAlgorithm` (`newStakingSession(params)`) and the evaluators (`PickSimulationService`, `BacktestService`) fold the session over the days via `PickBacktester.runStakedDay` — one session per run, days in ymd order, state keyed per `(year, round)`. Example: `FavoriteMartingaleAlgorithm` (`FAVORITE_MARTINGALE`), design in `docs/martingale-staking-design.md`. `pick_mstr.input_money` stores the per-slip stake as-is (no schema change needed for variable stakes).
