...
DDL is one file: `baseballApi/src/main/resources/db/ddl/schema.sql`, full CREATEs for a fresh database. `spring.jpa.hibernate.ddl-auto` is `none` and there is no Flyway/Liquibase — apply it by hand with the mysql CLI. The incremental `alter_*.sql` scripts that got the live database to this shape have all been applied and were deleted, since there is exactly one database and a spent migration can only mislead.

**`baseball_result` holds eight markets, not two**, one row per `(fixture, GAME_TYPE, COND)`, and every row now carries published pre-game odds:

| GAME_TYPE | rows | sides | overround |
|---|---|---|---|
| `야구 승1패` | 2,697 | 3 (승 / 1점차 / 패) | 1.1519 |
| `야구 전반 승무패` | 591 | 3 (real draws — 86) | 1.1521 |
| `야구 승패` | 2,676 | 2 (승 / 패) | 1.1412 |
| `야구 핸디캡` | 2,697 | 2, `COND = H ±1.5/2.5` | 1.1411 |
| `야구 언더오버` | 2,697 | 2 (언더 / 오버), `COND = U/O n.5` | 1.1413 |
| `야구 SUM` | 2,697 | 2 (홀 / 짝) | 1.1408 |
| `야구 전반 핸디캡` / `야구 전반 언더오버` | 591 each | 2 | 1.1415 |

The overrounds are **pooled averages, not constants** — the 2-way margin drifted from ~1.143 (260421~260712) to ~1.136 (260713 onward), a 0.7%p move that is the same size as the edges being hunted. Always de-vig per game through `ThreeWayOdds`/`TwoWayOdds.overround()`; never hard-code a margin.

Only the pick pipeline's two markets (`PickUniverse.THREE_WAY_GAME_TYPE`, `TWO_WAY_GAME_TYPE`) are wired to algorithms; every load filters on game type, so the other six are inert rather than polluting. They are the largest unexplored surface in the project — 핸디캡 and 언더오버 in particular are 2-way markets at the cheapest margin in the table, on signals (run totals, margin of victory) no current family looks at.
...
- **Read odds through `BaseballResult.publishedOdds()` / `publishedTwoWayOdds()` — the `PUB_*` columns are the only pre-game price a row has.** They return the odds actually posted before the game, or `null`, which means skip the game; there is deliberately no fallback. Which accessor answers depends on the row's market: a 3-way row fills all three columns, a 2-way row fills home and away and leaves `PUB_DRAW_DIV` null. The row's other odds, `totalDiv`, is the payout of the side that *won*, so it is known only afterwards and is never a selection signal — `PickSettlement` pays at it, and because published rows were cross-checked against it, selection and settlement quote the same market. De-vig through `impliedHome/Draw/Away()` rather than rolling your own, so every family measures the house margin the same way. Historical note: reconstructed `HOME_DIV`/`DRAW_DIV`/`AWAY_DIV` columns used to sit alongside these. Which of the three was exact depended on the outcome, so any rule reading them partly learned the result (`docs/statistical-model-design.md` 설계 결정 D2, and a fake +35~55% ROI). They were dropped rather than merely documented — if you find a reference to them, it is stale.
...
- **The choice of market matters more than most signals.** `야구 승1패` (3-way) settles `승` = home by 2+, `1` = **a one-run game** (not a draw — real ties are a handful of rows), `패` = away by 2+, so a favorite pick loses every one-run game and the market prices that slot at 22–28%. `야구 승패` (2-way) has no middle slot and a lower margin (1.1412 vs 1.1519). Same picks, same games: leg hit rate 41.4% → 56.4%, leg excess return about +6%p. Both rows carry their own `PUB_*`, so each market is priced from itself — read the 승패 row's `publishedTwoWayOdds()`, never the 3-way row's. A 2-way algorithm still needs `input.marketPairs()` (`MarketPairIndex`, keyed on `(year, round, tournament, ymd, tm, home, away)`) because the day and slot groupings are built on the 승1패 universe so the two families bet the same fixtures; it looks up the paired row, prices from it, and emits **that row's id** so settlement resolves and pays there. Never price a 2-way leg off its own `totalDiv` — only the realized side is measured, which is 설계 결정 D2 exactly. Attach the price as the `PickSelection`'s `BackedPrice` so the number that sized the stake is the number the benchmark uses; `PickSettlement` prefers it and otherwise reads the row's own quote. Examples: `FavoriteTwoWaySlipAlgorithm` (`FAVORITE_2WAY`), `FavoriteTwoWayMartingaleAlgorithm` (`FAVORITE_2WAY_MARTINGALE`), `WinRateOddsTwoWaySlipSelector` (`WIN_RATE_ODDS_2WAY`); analysis in `docs/statistical-model-design.md` §13–§14. Historical note: these three used to derive the 승패 price from the 3-way one through a `TwoWayOddsEstimator`, because 승패 rows had no `PUB_*`. Measured against the published prices that estimator was only 1.5–1.7% off on the number but named a **different favorite in 14.4% of games**, so it was deleted rather than kept as a fallback — approximately-right prices produce confidently-wrong picks. **An odds threshold does not carry across markets**: 2-way prices run 1.38–2.19 at the 5th–95th percentile against the 3-way 1.78–3.65, so a swept `x`/`y` range copied from a 3-way family sits outside the data and never binds — shift the range to the distribution, and compare against the price actually being taken. Form ranking is shared through `TeamRankSets.of(formIndex, ymd, num)`, so a 승패 variant differs from its 3-way sibling in exactly one place.
...
Endpoints (`/api/research`): `GET /goal`, `GET /algorithms`, `POST /search`, `POST /backtest`, `GET /leaderboard?limit=`, `GET /experiments`, `GET /forward-report`. Frontend: `/research` (`pages/research`).
...
- **`pick_forward`'s uniqueness key does not include `param_signature`** — it is `(algorithm_code, user_name, ymd, bucket, slip_no)`. Running a second frozen candidate under the same algorithm code and user on a day that already has slips silently collides on slip number and drops the tail instead of erroring. Give a second parallel candidate its own `userName` (e.g. `NSC_N1`); the report groups by `(algorithmCode, userName)` precisely so this stays possible.
...
- **Reporting is `research`'s job, not `pick`'s.** `pick` records and settles; `research/application/ForwardReportService` judges, because the goal, `BacktestMetrics`, and the ledger all live there and `pick` must not depend on `research`. The report deliberately reuses `BacktestMetrics` + `ExperimentGoal` rather than defining forward-only metrics or a forward-only bar: a separate scale would make "the backtest said +7%p, the forward test says +4%p" uninterpretable, and a separate bar would let the project pass a target it cannot pass in-sample. Expect it to fail the sample gates for weeks — that is the price of a number that cannot be a window effect.
- **Leg math comes off the record, not off today's data.** `ForwardPick.legTally()` / `benchmarkOutputMoney()` use each leg's stored `backedOdds` and `overround`, which is why that column exists. Re-deriving a price would replace the measurement with a reconstruction, and since the 2-way overround drifts (~1.143 → ~1.136), assuming a fixed margin mismeasures the benchmark by about as much as the edge being hunted. Pending slips contribute `LegTally.EMPTY` — an unresolved bet has not won or lost.
- **Two diagnostics the report exists to surface.** `backedSideCounts` catches a "form × odds" rule that has quietly become a one-sided bet, and `awayBaseline` scores backing away on the same legs so the form filter's contribution is separable rather than assumed — measured at **+9.46%p for the filter over blanket away** on 346 replayed legs (§17.2), which is what retired the "this is just fading the home team" objection. `coverage.missedYmds` is the operational one, and it is a *check this*, not a *lost*: it cannot distinguish "the loop did not run" from "the algorithm stood aside", since generation leaves no zero-slip marker.
- **Replaying past dates through this pipeline is a plumbing test, never a forward test.** Every day through 260817 was available while parameters were being chosen, so replaying it reproduces the backtest's contamination in a table named `pick_forward`. Run it under a throwaway `userName` and delete the output afterwards — a large in-sample record sitting in the forward table is the same trap the backfilled `*_DIV` columns were. The replay is worth running for what it does verify: `FixtureKey` settlement, bet-time price recording, and leg aggregation against real data (§17.2 — 346/346 legs resolved).
- **The forward path and the non-staking backtest path do not measure the same universe.** Forward generation always collapses `PickUniverse.distinctFixtures`; `BacktestService`'s `days` grouping (used for non-staking algorithms) deliberately does not, to keep ledger numbers comparable across history. Since 8.2% of fixtures are listed in two 회차, the backtest counts those as two independent legs when they are one game — which understates the standard error. Measured on the same window and params, dedup moved 372 legs → 346 and **t from 2.06 → 1.90**, i.e. the flat grouping alone can carry a candidate across the t ≥ 2.0 gate. Compare a forward number against a *staking-slot* backtest number, or expect this gap.

Endpoints: `GET /api/forward-picks` (list, filters `algorithmCode`/`userName`/`status`), `GET /api/forward-picks/schedule-dates`, `POST /api/forward-picks/generate`, `POST /api/forward-picks/settle?algorithmCode=&userName=`, plus `GET /api/research/forward-report` for the cumulative verdict. Frontend: `/forward-test` (`pages/forward-test` = `widgets/forward-test-report` + `widgets/forward-pick-list`).
